...
public class FiftyFiftyPanel extends PluginPanel {
    ...
    private void updateProgressPanel(Map<String, Integer> allKills) {
        // Get in-progress monsters (kills > 0 but < threshold)
    ...
    private void updateStatsPanel(Map<String, Integer> allKills, Map<String, Integer> thresholds) {
        ...
        // Calculate stats
        int totalKills = allKills.values().stream().mapToInt(Integer::intValue).sum();
        ...
        for (Map.Entry<String, Integer> entry : thresholds.entrySet()) {
            ...
            if (allKills.getOrDefault(mobName, 0) > mostKillCount) {
                mostKilled = mobName;
                mostKillCount = allKills.get(mobName);
            }
        }
        ...
        // Overall progress
        float overallProgress = thresholds.size() > 0 
            ? (float) completed / thresholds.size() 
            : 0;
        
        ProgressBar totalProgressBar = new ProgressBar();
        totalProgressBar.setMaximumValue(thresholds.size());
